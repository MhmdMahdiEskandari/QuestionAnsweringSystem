...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:@localhost/stackoverflow'
...
class user(db.Model):
	...
	score = db.Column('score',db.BIGINT)
	is_banned = db.Column('is_banned',db.Integer)
	is_superior = db.Column('is_superior',db.Integer)
...
class questions(db.Model):
	...
	hide = db.Column('hide',db.Integer,nullable=False)
	is_seen_by_superior = db.Column('is_seen_by_superior',db.Integer,nullable=False)
...
class user_que_vote(db.Model):
	...
	hide = db.Column('hide',db.Integer,default=0)
	que_vote_date = db.Column('que_vote_date',db.DateTime,nullable=False)
...
class user_ans_vote(db.Model):
	...
	downvote = db.Column('downvote',db.Integer,default=0)
	hide = db.Column('hide', db.Integer, default=0)
	ans_vote_date = db.Column('ans_vote_date', db.DateTime, nullable=False)
...
class user_interaction(db.Model):
	...
	FirstUserID = db.Column('FirstUserID',db.BIGINT,primary_key=True)
	SecondUserID = db.Column('SecondUserID',db.BIGINT)
	InteractionValue = db.Column('InteractionValue',db.BIGINT,default=0)
